  ...
  render: function () {
    ...
    return (
      <ul
        ...
        >
      {this.traverseChildren(this.props.children, this.renderNavItem)}
      </ul>
    );
  },
  //traverse children
  //Calculate the amount of MenuItem in all children
  traverseChildren: function (children, func, context) {
    this.menuItemNum = 0;

    return React.Children.map(children, function (child) {
      if (React.isValidElement(child)) {

        if( child.type.displayName == 'MenuItem' && !child.props.divider ){
          var lastIndex = this.menuItemNum;
          this.menuItemNum++;
